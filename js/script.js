...
initDashboard();
/* =========================================
   HOME QA 2.0 — ESTADO VISUAL DEL MODELO.
   Activa o desactiva el botón "Cambiar modelo".
========================================= */

const changeModelButton = document.querySelector("#change-model-button");
const modelCards = document.querySelectorAll(".model-card");
const modelStartSection = document.querySelector("#model-start");
const workflowSection = document.querySelector("#workflow");

function enableChangeModelButton() {
    document.body.classList.add("has-active-model");
}

function disableChangeModelButton() {
    document.body.classList.remove("has-active-model");
}

modelCards.forEach((card) => {
    card.addEventListener("click", () => {
        enableChangeModelButton();
    });
});

if (changeModelButton) {
    changeModelButton.addEventListener("click", () => {
        disableChangeModelButton();
    });
}
/* =========================================
   HOME QA 2.0 — ESTADO VISUAL DEL BOTÓN CAMBIAR MODELO.
   Bloque aislado para no romper la lógica existente.
========================================= */

(() => {
    const homeResetButton = document.querySelector("#reset-workflow-button");
    const homeCards = document.querySelectorAll(".model-card");

    const activateResetButton = () => {
        document.body.classList.add("has-active-model");

        if (!homeResetButton) return;

        homeResetButton.disabled = false;
        homeResetButton.setAttribute("aria-disabled", "false");
    };
    ...
    const deactivateResetButton = () => {
        ...
        homeResetButton.disabled = true;
        homeResetButton.setAttribute("aria-disabled", "true");
    };

    homeCards.forEach((card) => {
        card.addEventListener("click", activateResetButton);
    });

    if (homeResetButton) {
        homeResetButton.addEventListener("click", deactivateResetButton);
    }
})();

/* =========================================
   HOME QA 2.0 — LOGO VUELVE AL HOME.
   Hace que el logo reinicie el workflow y vuelva al selector.
========================================= */

(() => {
    const brandLink = document.querySelector(".app-header__brand");
    const resetButton = document.querySelector("#reset-workflow-button");
    const modelStart = document.querySelector("#model-start");
...
/* =========================================
   WORKFLOW QA 2.0 — BODY CLASS POR MODELO.
========================================= */

(() => {
    const modelCards = document.querySelectorAll(".model-card");
    const resetButton = document.querySelector("#reset-workflow-button");
    const modelClasses = ["model-rapido", "model-controlado", "model-profundo"];

    const clearModelClasses = () => {
        document.body.classList.remove(...modelClasses);
    };
